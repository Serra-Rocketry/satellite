...
aerodynamic model, implemented in RocketPy [\ref{ref6}] with SRAB extension
...
(Fig.~\ref{fig:lrr-asa3}) and the satellite-view trajectory map
(Fig.~\ref{fig:trajectory-map}):
...
Three instrumented drop-test campaigns validated the model, with the
results summarized in Table~\ref{tab:droptests}:
...
\bottomrule
\end{tabular}
\caption{Instrumented drop-test results.}
\label{tab:droptests}
...
becoming the intermediate candidate. Test 3 (Asa3, 4 wings,
1000~m drop, simulated) reached steady autorotation at 12.90~m/s in 78.03~s,
...
(10.05~m/s), which increases the margin against the 20~m/s limit.
However, they pay for this in mass, rotating inertia and mechanical
