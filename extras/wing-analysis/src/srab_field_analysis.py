    ...
    for ph in phases:
        if ph["name"] not in seen:
            handles.append(Patch(
                ...
                alpha=0.4, label=ph["name"]
            ))
            seen.add(ph["name"])
    ax.legend(handles=handles, fontsize=8, loc="upper right")


def plot_dashboard(data: dict, phases: list, metrics: dict,
                   sim: dict | None, out_path: Path):
    ...
    if sim is not None:
        ...
        t_peak = t[i_peak]
        ax.plot(sim["t"] + t_peak, sim["alt"] - sim["alt"][0],
                color="#378ADD", lw=1.2, ls="--", alpha=0.55, label="Sim.")
    ...
    if sim is not None:
        i_peak = np.argmax(alt)
        t_peak = t[i_peak]
        ax.plot(sim["t"] + t_peak, sim["v0"],
                color="#E24B4A", lw=1.2, ls="--", alpha=0.55, label="Sim. v₀")
    ...
    ax.plot(t, np.abs(gz) * 60 / (2 * np.pi),
            color="#1D9E75", lw=1, ls="--", alpha=0.55, label="|gz| (RPM)")
    if sim is not None:
        i_peak = np.argmax(alt)
        t_peak = t[i_peak]
        ax.plot(sim["t"] + t_peak, sim["spin"],
                color="#1D9E75", lw=1.2, ls=":", alpha=0.55, label="Sim. spin")
    ax.set_ylabel("RPM")
    ...
    ax.plot(t, data["az"], color="#8e44ad", lw=1, ls="--", alpha=0.55, label="az (m/s²)")
    ...
    tbl = ax.table(cellText=rows_table[1:], colLabels=rows_table[0],
    ...
    for ax_i in fig.get_axes():
        if not ax_i.get_title().startswith("Métricas"):
            for ph in phases:
                ax_i.axvline(ph["t_start"], color=PHASE_COLORS.get(ph["name"], "#999"),
                             lw=0.8, ls="-", alpha=0.5)
...
def print_report(data: dict, phases: list, metrics: dict, sim: dict | None):
    ...
    print(f"  Δ altitude:       {g['delta_alt_m']:.2f} m  "
    ...
    if desc:
        ...
        print(f"  Janela LASC:        20–45 m/s  →  {'✅ dentro' if lasc_ok else '⚠️  fora'}")
...
def parse_args():
    ...
    ap.add_argument("--sim",   type=Path, default=None, metavar="CSV",
    ...
    ap.add_argument("--vz-source", choices=["baro", "imu"], default="baro",
                    help="Fonte de vz: 'baro' = barômetro (padrão), 'imu' = integração az")
    ap.add_argument("--spin-axis", choices=["mag", "gz"], default="mag",
...
def main():
    ...
    print(f"  {data['n']} amostras  |  {data['freq']:.1f} Hz  |  "
