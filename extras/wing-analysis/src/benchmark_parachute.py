...
from samara_pq_simulation import PocketQubeSamaraWing, PocketQubeFlightDynamics, PocketQubeMissionReporter
...
wing = PocketQubeSamaraWing(
    dxf_path=DXF, n_wings=2, mass=MASS_KG,
...
mass_para = 0.005  # massa do paraquedas [kg]
total_mass = MASS_KG + mass_para
...
print(f"  m_para:     {mass_para*1000:.0f} g")
...
print(f"  {'-'*25} {'-'*12} {'-'*12} {'-'*12}")
print(f"  {'v_impacto [m/s]':<25} {v_srab:>12.2f} {v_impact_para:>12.2f} {v_srab - v_impact_para:>+12.2f}")
print(f"  {'t_descida [s]':<25} {t_srab:>12.2f} {t_impact_para:>12.2f} {t_srab - t_impact_para:>+12.2f}")
print(f"  {'KE_impacto [J]':<25} {ke_srab:>12.2f} {ke_impact_para:>12.2f} {ke_srab - ke_impact_para:>+12.2f}")
print(f"  {'LASC 20-45 m/s':<25} {'DENTRO' if 20 <= v_srab <= 45 else 'FORA':>12} {'DENTRO' if 20 <= v_impact_para <= 45 else 'FORA':>12}")
print("=" * 65)
...
print(f"| Dentro da janela LASC (20-45 m/s) | {'Sim' if 20 <= v_srab <= 45 else 'Nao'} | {'Sim' if 20 <= v_impact_para <= 45 else 'Nao'} |")
